...
class Model:
    ...
    def loss(probabilities, gt_label):
        ...
        loss = -np.log(prob_0[gt_label] + 1e-10)
        return loss
    ...
    def back_propagation(img, probabilities, gt_label):
        ...
        prob_0 = probabilities[0]
        #subtract 1 from p_j only if it is the true class probability:
        prob_0[gt_label] = prob_0[gt_label] - 1
        # TODO: Reshape train image data to be matrix, dimension [784, 1]
        #print(img.shape)
        img = np.expand_dims(img, axis=1)
        gradW, gradB = np.dot(img, probabilities), probabilities
        return gradW, gradB
    ...
    def train_nn(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for index in range(len(indices)):
                ...
                loss = self.loss(probabilities, gt_label)
                loss_sum += loss
